...
export const getFilterInfoByKey = <
...
>(
  ...
  setFilterMeta(
    Base,
    {
      ...
      sort: Object.assign(
        { latest: { createdAt: -1 }, oldest: { createdAt: 1 } },
        querySort.sort,
      ),
    },
    ...
    any,
    {
      [FILTER_META_KEY]: {
        query: BaseQuery<Full> &
          MergeAllDoubleKeyOfObjects<
            LibFilters,
            typeof FILTER_META_KEY,
            "query"
          > &
          _Filter["query"];
        sort: BaseSort &
          MergeAllKeyOfTypes<LibFilters, "sort"> &
          _Filter["sort"];
      };
    }
  >;
...
> {
  ...
  >(
    name: ArgName,
    argRef: Arg,
    option?: Omit<ArgProps<_FieldToValue>, "nullable">,
  ) {
    ...
    return this as unknown as FilterInfo<
      [...ArgNames, ArgName],
      [...Args, arg: _FieldToValue],
      Model
    >;
  }
  opt<
