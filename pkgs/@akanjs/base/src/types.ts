...
export type Cls<T = any, Statics = unknown> = (new (...args: any[]) => T) &
  Statics;
...
export type UnCls<T> = T extends new (...args: any) => infer U ? U : never;
...
export type MergeAllActionTypes<
  T extends Cls[],
  OmitKey extends string = never,
> = T extends [infer First extends Cls, ...infer Rest extends Cls[]]
  ? Rest extends []
    ? Omit<GetActionObject<UnCls<First>>, OmitKey>
    : MergeAllActionTypes<Rest, OmitKey> & Omit<UnCls<First>, OmitKey>
  : unknown;

export type MergeAllTypes<
  T extends Cls[],
  OmitKey extends string = never,
> = T extends [infer First extends Cls, ...infer Rest extends Cls[]]
  ? Rest extends []
    ? Omit<UnCls<First>, OmitKey>
    : MergeAllTypes<Rest, OmitKey> & Omit<UnCls<First>, OmitKey>
  : unknown;

export type MergeAllKeyOfTypes<
  T extends Cls<{ [key: string]: any }>[],
  Key extends string,
> = T extends [infer First extends Cls, ...infer Rest extends Cls[]]
  ? Rest extends []
    ? UnCls<First>[Key]
    : MergeAllKeyOfTypes<Rest, Key> & UnCls<First>[Key]
  : unknown;
...
export type SingleValue<Value> = Value extends (infer V)[]
  ? SingleValue<V>
  : Value;

export type StrArrToObject<Arr, Value> = Arr extends [
  infer First,
  ...infer Rest,
]
...
export type GetValueOfKey<
  Model,
  Key extends string,
  Fallback = never,
> = Model extends { [key in Key]: any } ? Model[Key] : Fallback;
