...
const objectModelOf = <T>(
  ...
  const field = Object.assign(
    ...
    inputRef.field,
    Object.fromEntries(
      Object.entries(fieldMap).map(([key, fieldInfo]) => [
        key,
        fieldInfo.toField(),
      ]),
    ),
...
const lightModelOf = <T, F extends keyof OmitBase<T>>(
  ...
  const field = Object.assign(
    ...
    Object.fromEntries(
      Object.entries(fieldMap).map(([key, field]) => [key, field.toField()]),
    ),
    Object.fromEntries(
      fields.map((field) => [field, objectRef.field[field as string]]),
    ),
  );
...
const fullModelOf = <A, B = undefined>(
  ...
  lightRef: Cnst<B>,
  fieldMap: FieldInfoObject,
  ...libFullModelRefs: Cnst[]
): Cls<Merge<A, B>> => {
  const fullRef =
    libFullModelRefs.at(0) ??
    getBaseConstantClass(ConstantField.getBaseModelField());
  Object.assign(
    fullRef.field,
    objectRef.field,
    lightRef.field,
...
const getBaseConstantClass = (
  field: FieldObject,
  modelType: ModelType = "scalar",
) => {
  class BaseConstant {
    static readonly field: FieldObject = field;
    static modelType: ModelType = modelType;
    [immerable] = true;
    constructor(obj?: any) {
      this.set({
        ...(this.constructor as Cnst).default,
    ...
    set(obj: Partial<typeof this>) {
      Object.entries(obj).forEach(([key, value]) => {
        //check field has key
        if (
          !(this.constructor as Cnst).field[key] as unknown as
            | object
            | undefined
        )
          return;
        const field = (this.constructor as Cnst).field[key];
        if (!field) throw new Error(`Field ${key} not found`);
        const fieldProp = field.getProps();
        (this as { [key: string]: any })[key] = crystalize(
          fieldProp,
          value,
        ) as object;
      });
...
export type Cnst<Schema = any> = (new (
  obj?: Partial<Schema>,
) => Crystal<Schema>) &
...
const applyConstantStatics = <Model>(
  model: Cls<Model, { field: FieldObject }>,
): Cnst<Model> => {
  Object.assign(model, {
  ...
  return model as unknown as Cnst<Model>;
};
...
export function via<
  ...
  _Schema = MergeAllTypes<LightModels> &
    Pick<T, K> &
    BaseObject &
    ExtractFieldInfoObject<ReturnType<ResolveField>>,
>(
  modelRef: Cls<T>,
  fields: readonly K[],
...
export function via<
  ...
  Inputs extends Cls[],
  _Schema = MergeAllTypes<Inputs> &
    ExtractFieldInfoObject<ReturnType<BuildField>>,
>(buildField: BuildField, ...extendInputRefs: Inputs): Cnst<_Schema>;
...
export function via<
  ...
  _Schema = MergeAllTypes<Insights> &
    BaseInsight &
    ExtractFieldInfoObject<ReturnType<BuildField>>,
>(
  modelRef: Cls<T>,
...
export function via<
  ...
  _Schema = MergeAllTypes<ObjectModels> &
    T &
    BaseObject &
    ExtractFieldInfoObject<ReturnType<BuildField>>,
>(
...
export function via<
  ...
  _Schema = MergeAllTypes<FullModels> &
    T &
    Light &
    ExtractFieldInfoObject<ReturnType<ResolveField>>,
>(
...
export function via(
  ...
  secondRefOrFieldsOrBuildField?:
    | Cls
    | readonly any[]
    | ((builder: FieldBuilder) => FieldInfoObject),
  ...
  if (
    ...
    !(firstRefOrBuildField as Cls<any, { modelType?: ModelType }>).modelType
  ) {
    const buildField = firstRefOrBuildField as (
      builder: FieldBuilder,
    ) => FieldInfoObject;
    const fieldMap = buildField(field);
    const extendInputRefs = [
      ...(secondRefOrFieldsOrBuildField
        ? [secondRefOrFieldsOrBuildField as Cls]
        : []),
  ...
  if (Array.isArray(secondRefOrFieldsOrBuildField)) {
    const resolveField = thirdRefOrResolveField as (
      resolve: FieldResolver,
    ) => FieldInfoObject;
    const fieldMap = resolveField(resolve);
    return lightModelOf(
      firstRefOrBuildField as Cnst,
  ...
  if (
    ...
    !(secondRefOrFieldsOrBuildField as Cls<any, { modelType?: ModelType }>)
      .modelType
  ) {
    const buildField = secondRefOrFieldsOrBuildField as (
      builder: FieldBuilder,
    ) => FieldInfoObject;
    const fieldMap = buildField(field);
    // object via
    if (
      ConstantRegistry.isScalar(
        firstRefOrBuildField as Cls<any, { modelType: ModelType }>,
      )
    ) {
      if (!thirdRefOrResolveField)
        return objectModelOf(firstRefOrBuildField as Cnst, fieldMap);
      else
        return extendModelObjects(
          firstRefOrBuildField as Cnst,
          fieldMap,
          thirdRefOrResolveField as Cnst,
          ...(extendRefs as Cnst[]),
        );
    }
    // insight via
    if (
      ConstantRegistry.isFull(
        firstRefOrBuildField as Cls<any, { modelType: ModelType }>,
      )
    ) {
    ...
    const objectRef = firstRefOrBuildField as Cnst;
    const lightRef = secondRefOrFieldsOrBuildField as Cnst;
    const resolveField = thirdRefOrResolveField as (
      resolve: FieldResolver,
    ) => FieldInfoObject;
    const fieldMap = resolveField(resolve);
...
const extendModelInputs = <T extends Cnst[]>(
  fieldMap: FieldInfoObject,
  ...libInputModelRefs: T
): Cls<MergeAllTypes<T>> => {
...
const extendModelObjects = <Input, ObjectModels extends Cnst[]>(
  inputRef: Cnst<Input>,
  fieldMap: FieldInfoObject,
...
const extendModelInsights = <InsightModels extends Cnst[]>(
  fieldMap: FieldInfoObject,
