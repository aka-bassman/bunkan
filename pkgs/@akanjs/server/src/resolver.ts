...
import { Cnst, constantInfo, GqlReturn } from "@akanjs/constant";
...
const getNestReturn = (returns: GqlReturn, type: "input" | "object" = "object") => {
  const [model, arrDepth] = getNonArrayModel(returns() as Cnst);
...
export const resolverOf = (sigRef: Type, allSrvs: { [key: string]: Type }) => {
  ...
  if (sigMeta.returns) {
    ...
    Object.entries(modelRef.field)
      .filter(([key, field]) => field.isClass && !field.isScalar && !resolveFieldKeys.includes(key))
      .forEach(([key, field]) => {
        const refName = constantInfo.getRefName(field.modelRef);
        const className = capitalize(refName);
        const serviceName = `${refName}Service`;
        Rsv.prototype[key] = async function (this: { [key: string]: any }, parent: { [key: string]: object }) {
          const service = this[serviceName] as { [key: string]: (...args) => Promise<object[]> };
          return field.arrDepth
            ? await service[`load${className}Many`](parent[key])
            : await service[`load${className}`](parent[key]);
        };
        Nest.Parent()(Rsv.prototype, key, 0);
        Nest.ResolveField(getNestReturn(() => arraiedModel(field.modelRef, field.arrDepth) as Type))(
          Rsv.prototype,
          key,
