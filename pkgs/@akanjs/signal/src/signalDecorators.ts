...
import type {
  EndpointServerSignal,
  InternalServerSignal,
  ServiceModule,
} from "@akanjs/service";
...
import {
  BuildInternalApiSignal,
  InternalApiBuilder,
  makeInternalApiBuilder,
} from "./internalApiInfo";
import { signalInfo } from "./signalInfo";

export class SignalStorage {}

export const getAllSignalRefs = () => {
  const signalNames = Reflect.getOwnMetadataKeys(SignalStorage.prototype) as
    | string[]
    | undefined;
  const sigRefs =
    signalNames?.reduce<Cls[]>(
      (acc, signalName) => [...acc, ...getSignalRefsOnStorage(signalName)],
      [],
    ) ?? [];
  return sigRefs;
};
export const getSignalRefsOnStorage = (refName: string) => {
  const sigRefs = Reflect.getMetadata(refName, SignalStorage.prototype) as
    | Cls[]
    | undefined;
  return sigRefs ?? [];
};
...
export const setSignalRefOnStorage = (refName: string, signalRef: Cls) => {
  Reflect.defineMetadata(
    refName,
    [...getSignalRefsOnStorage(refName), signalRef],
...
type ExtendedGqlReturn<
  Return,
  Full,
  Light,
  Insight,
  _UnTypeReturn = UnCls<Return>,
> = Return extends (infer R)[]
...
>(
  ...
  Assign<
    ReturnType<Builder>,
    ExtendEndpoints<Full, Light, Insight, LibEndpoints>
  >
> {
...
export const serverSignalOf = <Endpoint, Internal>(
  endpointRef: Cls<Endpoint>,
  internalRef: Cls<Internal>,
  sigRef: Cls,
): Cls<EndpointServerSignal<Endpoint> & InternalServerSignal<Internal>> => {
  return sigRef as any;
};
