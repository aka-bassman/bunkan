...
import type { ServiceModule, BaseService } from "@akanjs/service";
...
// !FIXME: signal 패키지가 완전히 서버사이드로 이전되면 srv.base로 사용
const srvBase = { refName: "base" } as ServiceModule<
  "base",
...
export class BaseEndpoint extends endpoint(
  srvBase,
  ({ query, mutation, message, pubsub }) => ({
    ...
    getDictionary: query(Any)
      .param("lang", String)
      .exec(function (lang) {
        const dictionary = getDictionary(lang as "en") as Record<
          string,
          object
        >;
        return dictionary;
    ...
    getAllDictionary: query(Any).exec(function () {
      const dictionary = getAllDictionary() as Record<
        string,
...
export class Base extends serverSignalOf(
  BaseEndpoint,
  BaseInternal,
  BaseSignal,
) {}
