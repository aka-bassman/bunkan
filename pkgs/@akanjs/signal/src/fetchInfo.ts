          ...
          try {
            const res = (
              (await (endpoint.type === "query" ? query : mutate)(
                ...
                fetchPolicy
              )) as unknown as { [key: string]: object | null }
            )[name];
            const data = resolve && res !== null ? (crystalize(res) as object) : res;
    ...
    [names.mergeModel]: async (modelOrId: Full | string, data: Partial<Full>, option?: FetchPolicy) => {
      const model =
        typeof modelOrId === "string"
          ...
          : modelOrId;
      const input = (cnst.input.purify as (m: any) => Input | null)({
        ...model,
        ...data,
      });
    ...
    signal.slices.reduce((acc, { sliceName, argLength, defaultArgs }) => {
      ...
      const initFn = async function (...args: any[]) {
        ...
        const modelList = new DataList(
          modelObjList.map((modelObj) => new cnst.light().set(modelObj) as unknown as Light)
        );
        const modelInsight = new cnst.insight().set(modelObjInsight) as unknown as Insight;
        return {
