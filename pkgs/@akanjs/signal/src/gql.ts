...
import { FetchPolicy, Logger } from "@akanjs/common";
...
export type ServerView<T extends string, Model> = { refName: T } & { [K in `${T}Obj`]: GetStateObject<Model> } & {
  [K in `${T}ViewAt`]: Date;
};
export type ClientView<T extends string, Model> = PromiseOrObject<ServerView<T, Model>>;

export type ServerEdit<T extends string, Model> = { refName: T } & { [K in `${T}Obj`]: GetStateObject<Model> } & {
  [K in `${T}ViewAt`]: Date;
};
...
type EmitMethods<Sig> = {
  [K in keyof Sig as HasMarker<Sig[K], "Emit"> extends true ? K : never]: Sig[K] extends (...args: infer Args) => any
    ? (...args: [...Args, fetchPolicy?: FetchPolicy]) => void
    : never;
};
...
export const query = async <Query = any>(
  ...
  const { data, error } = await fetchClient.gql
    ...
    .toPromise();
  if (!data) {
    Logger.rawLog(error as unknown as string, undefined, "error");
